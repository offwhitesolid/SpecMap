...
// test data set instead of "real data": 
...
function LoadPanel()
    ...
    Make/O/N=29 root:Packages:myFolder:Pathnum
    ...
    Pathnum[3] = 10
    Pathnum[4] = 150
    NewPanel /W=(81,73,850,248)/N=Load_Panel
...
function ProcessPanel()
    ...
    Button PlotfitCA, pos={200.00,35.00},size={180.00,20.00},proc=ButtonProc,title="Plot spec & fit under Cursor A",win=Process_Panel
    // listbox to select fit function
    variable roiselect = 65
    ...
    variable fitbuttonstart = roiselect + 150
    Button FitHSIcount, pos={13.00,fitbuttonstart},size={180.00,20.00},proc=ButtonProc,title="Fit Function to hsi count select",win=Process_Panel
    Button FitHSIroi, pos={13.00,fitbuttonstart+25},size={180.00,20.00},proc=ButtonProc,title="Fit Function to hsi roi select",win=Process_Panel
	...
	Button FitFunctionplot, pos={13.00,fitbuttonstart+50},size={180.00,20.00},proc=ButtonProc,title="Plot Fit and Spec under cursor",win=Process_Panel
    ...
    String ctrlName
        ...
        strswitch(ctrlName)
            ...
            case "plotselpixm":
            	...
            	wave/T pixmw = $"root:packages:myfolder:pixmatw"
				pixsel = getpixmatselect()
				plotpixmatrix(pixmw[pixsel]) // for different plot look: copy the plotpixmatrix function and substitute it here
            	break
            
            case "plavpixm": // variables: see case "plotselpixm" above
            	wave/T pixmw = $"root:packages:myfolder:pixmatw"
	            pixsel = getpixmatselect()
				AveragePixmatrixSpecs(pixmw[pixsel])
            	break
            
            case "FitParameterplot": // create HSI from fitparameter
            	fitparameter2pximatrix(1, getfitboxselect())
            	buildpixmatrixselect()
            	break
...
// create pixmatrix from hsi fit parameter
function fitparameter2pximatrix(showimage, fitwaveN)
	variable showimage
	variable fitwaveN
	updateWLInput()
	wave/T path = root:Packages:myFolder:Path
	wave pathnum = root:Packages:myFolder:Pathnum
	wave fitstorewave = root:
	...
	wave fitmat = root:HSI:spec:fitparameter
...
function HSIplotspec(x, y, plotfit)
	...
	Label left "PL / counts"
	if (plotfit == 1)
		if (getfitboxselect() == 1)
			setdatafolder root:packages:Fitdata
			wave fitwave = root:hsi:
			make/O/N=(numpnts(WL)) fitdispw
			make/O/N=4 fitparametr
			wave fp = fitparameter
			//fp[0] = 
			//fp[1] =
		endif
	endif
...
function getpixmatselect()
	variable pixsel = 0
	variable i
	wave rois = root:packages:myfolder:pixmatselect
	if (numpnts(rois)== 0)
		return Nan
	endif
	for (i=0; i<numpnts(rois); i+=1)
...
function getfitboxselect()
	variable pixsel = 0
	variable i
	wave rois = root:packages:myfolder:fitboxselect
	if (numpnts(rois)== 0)
		return Nan
	endif
	for (i=0; i<numpnts(rois); i+=1)
...
function fittohsi(selectspec)
	variable selectspec //0: select by counts, 1: select by selected roi (NaN wont be fitted)
	wave rois = root:packages:myfolder:roisw
	if (selectspec == 1)
		if (numpnts(rois)== 0)
			selectspec = 0
		else 
		variable roisel = getroiselect()
		endif
	endif
	...
	variable fitcurrpixel = 0 // fit the current pixel
		...
		for (j=0; j<numpnts(gridy); j+=1)
			if (hsiint[i][j] >= pathnum[14]) // fit only if pixel in pixmatrix1 > count threshold for fit
				...
				if (fitselect[0] == 1) // gaussian fit Gauss: fitw[0-7]
					curvefit/Q=1 gauss, fitwave /X=fitwl
					wave w_coef, w_sigma
					fitw[i][j][0] = w_coef[0]
					fitw[i][j][1] = w_coef[1]
					fitw[i][j][2] = w_coef[2]
				...
				elseif (fitselect[1] == 1) // lorenz fit lorentzian: fitw[8-15]
					curvefit/Q=1 lor, fitwave /X=fitwl
					fitw[i][j][8] = w_coef[0]
					fitw[i][j][9] = w_coef[1]
				...
				elseif (fitselect[2] == 1) // voigt fit voigt: fitw[16-25]
					curvefit/Q=1 voigt, fitwave /X=fitwl
					wave w_coef, w_sigma
