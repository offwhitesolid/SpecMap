...
class ScaleBarApp:
    def __init__(self, root, magnification=5, max_display_size=(800, 600)):
        self.root = root
    ...
    def create_draggable_scale_bar(self):
        ...
        self.bar_length_mum = 10
        self.bar_hight_mum = 1
        self.bar_length_pix = int(self.bar_length_mum / self.scalepixels)
        self.bar_height_pix = int(self.bar_hight_mum / self.scalepixels)
        self.text = f"{self.bar_length_mum} µm"
        
        # Default position of the scale bar on the resized display image
        self.bar_x = self.display_image.width - self.bar_length_pix - 10
        self.bar_y = 10
        
        # Draw initial scale bar and text
        self.bar = self.canvas.create_rectangle(self.bar_x, self.bar_y, self.bar_x + self.bar_length_pix, self.bar_y + self.bar_height_pix, fill="white")
        self.text_id = self.canvas.create_text(self.bar_x, self.bar_y + self.bar_height_pix + 10, anchor="nw", text=self.text, fill="white")
    ...
    def save_image(self):
        ...
        # Reload the original high-resolution image
        original_image = Image.open(self.image_path)
        
        # Calculate scale bar position and size in original image coordinates
        original_bar_x = int(self.bar_x * self.scale_ratio)
        original_bar_y = int(self.bar_y * self.scale_ratio)
        original_bar_length_pix = int(self.bar_length_pix * self.scale_ratio)
        original_bar_height_pix = int(self.bar_height_pix * self.scale_ratio)
        ...
        draw.rectangle([original_bar_x, original_bar_y, original_bar_x + original_bar_length_pix, original_bar_y + original_bar_height_pix], fill="white")
        
        # Load font for the text
        try:
            font = ImageFont.truetype("arial.ttf", int(12 * self.scale_ratio))  # Scale font size accordingly
        except IOError:
            font = ImageFont.load_default()
        
        # Draw the text below the scale bar
        draw.text((original_bar_x, original_bar_y + original_bar_height_pix + int(5 * self.scale_ratio)), self.text, fill="white", font=font)
