...
class ScaleBarApp:
    ...
    def create_draggable_scale_bar(self):
        ...
        self.bar = self.canvas.create_rectangle(
            self.bar_x, self.bar_y, self.bar_x + self.bar_length, self.bar_y + self.bar_height, fill="white"
        )
        self.text_id = self.canvas.create_text(
            self.bar_x, self.bar_y + self.bar_height + 10, anchor="nw", text=self.text, fill="white"
        )
