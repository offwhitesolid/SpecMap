...
This document outlines the rigorous mathematical foundation for evaluating spectral derivatives using local polynomial fits, and demonstrates exactly how it translates to the ultra-fast **Savitzky-Golay** method used in the benchmarking scripts.
...
**The Solution:** The data must be smoothed concurrently with taking the derivative. Fitting a smooth polynomial to a local window of points accomplishes exactly this.
...
Let the coordinate system be translated locally so the center point is at $x = 0$. 
The input coordinates for this window are $x_i$ where $i \in \{-m, \dots, -1, 0, 1, \dots, m\}$.
...
**Conclusion:** The first derivative at the center of the sliding window is *exactly* the $c_1$ coefficient of the least-squares polynomial.

---

## 5. The Savitzky-Golay Method: Pre-Computing the Convolution Weights

If the above is implemented naively using `np.polyfit` in a sliding loop, the algorithm has to construct the matrix $\mathbf{X}$, calculate $\mathbf{X}^T \mathbf{X}$, invert it, and multiply it by $\mathbf{X}^T \mathbf{y}$ for **every single wavelength pixel** in the spectrum. This requires solving an $(n+1) \times (n+1)$ matrix system hundreds or thousands of times per array!
...
To understand exactly *why* finite differences fail on physical spectra and why the polynomial (Savitzky-Golay) approach succeeds, the variance of the noise must be mathematically evaluated.
...
Therefore, as the polynomial window size $N$ is increased, **the noise variance drops dramatically according to $O(1/N^3)$.** The polynomial fit actively averages out the random fluctuations across the entire window rather than amplifying the raw difference between just two adjacent noisy pixels. This allows for the construction of virtually noiseless, ultra-reliable derivatives of highly sensitive physical spectra.